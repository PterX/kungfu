...
OrderBookLevel = recordclass("OrderBookLevel", LEVEL_FIELDS)
...
class OrderBookUtils(object):
    ...
    def csv_book_update(csvfile, book):
        ...
        row.update({LEVEL_FORMAT.format(1, "bid", f): bid[f] for f in LEVEL_FIELDS})
        row.update({LEVEL_FORMAT.format(1, "offer", f): offer[f] for f in LEVEL_FIELDS})
    ...
    def csv_trade_update(csvfile, book, trades):
        ...
        for trade in trades:
            ...
            row.update({LEVEL_FORMAT.format(1, "bid", f): None for f in LEVEL_FIELDS})
            row.update({LEVEL_FORMAT.format(1, "offer", f): None for f in LEVEL_FIELDS})
    ...
    def bin_book_update(binfile, book):
        ...
        valids_data = [2]
        the_data = (
            [now_nanos(), book.security]
            + trade_update_data
            + order_book_level_data
            + valids_data
        )
        data = struct.pack(
            "<QI" + trade_update_fmt + order_book_level_fmt * 5 + valids_fmt, *the_data
        )
        binfile.write(data)
...
class OrderBook(object):
    ...
    def match(self, aggressor_side):
        ...
        for bid_i in range(len(self.bid) - 1, -1, -1):
            ...
            while offer_i < size_offer:
                ...
                if crossed:
                    trade = Trade(
                        price=offer.price, qty=offer.qty, aggressor=aggressor_side
                    )
    ...
    def order(self, order):
        ...
        if self.security != order.secid:
            raise (
                "Cannot place order for security "
    ...
    def display(self):
        ...
        size_offer = len(self.offer)
        print(
            "Book[%s]: %d bids, %d offers --> mid @ %f"
            % (self.security, size_bid, size_offer, self.mid())
        )
    ...
    def gen_orders(self, config):
        ...
        mid = (
            config.base + random.randint(-config.bound + 1, config.bound - 1) * MIN_TICK
        )
        direction = 1.0
        ...
        for i in range(config.samples):
            ...
            orders += OrderBookUtils.pad_book(
                self, self.depth_bids(), buy_price, Side.BUY
            )
            orders += OrderBookUtils.pad_book(
                self, self.depth_offers(), sell_price, Side.SELL
            )
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        "--instruments",
        dest="instruments",
        help="Security IDs, space separated list",
        type=int,
        nargs="+",
        action="store",
        default=666666,
...
def main(argv):
    ...
    print(
        "Output file '{}' in {} format".format(
            args.outputfile.name, "CSV" if args.csv else "binary"
        )
    )
