...
public:
  ...
  bool subscribe(const std::vector<longfist::types::InstrumentKey>
                     &instrument_keys) override;

  bool subscribe_all() override;
  bool
  subscribe_custom(const longfist::types::CustomSubscribe &custom_sub) override;
  bool unsubscribe(const std::vector<longfist::types::InstrumentKey>
                       &instrument_keys) override {
    return false;
  };
...
private:
  ...
  inline static thread_local yijinjing::journal::writer_ptr
      entrust_band_writer_{};
  inline static thread_local yijinjing::journal::writer_ptr
      transaction_band_writer_{};
  ...
  std::map<std::string, double>
      map_volume_{}; // 存放所有标的最后一笔撮合的volume数据

  std::map<std::string, std::map<double, Level>>
      bid_orderbooks_{}; // 存放所有标的的买单数据
  std::map<std::string, std::map<double, Level>>
      ask_orderbooks_{}; // 存放所有标的的买单数据
  static inline std::string
  make_exchange_instrument(const std::string &exchang_id,
  ...
  void write_transaction_trade(const longfist::types::Entrust &entrust,
                               double volume,
                               int64_t order_no); // 写成交类型的Transaction

  template <class T>
  void add_subscribe(const longfist::types::InstrumentKey &key,
                     std::map<std::string, T> &map_) {
    auto pair = map_.try_emplace(
        make_exchange_instrument(key.exchange_id, key.instrument_id));
    if (pair.second) {
      T &data = pair.first->second;
      data.exchange_id = key.exchange_id;
      data.instrument_id = key.instrument_id;
      data.instrument_type =
          get_instrument_type(key.exchange_id, key.instrument_id);
    }
    bid_orderbooks_.try_emplace(
        make_exchange_instrument(key.exchange_id, key.instrument_id));
    ask_orderbooks_.try_emplace(
        make_exchange_instrument(key.exchange_id, key.instrument_id));
  }
};
} // namespace kungfu::wingchun::sim
