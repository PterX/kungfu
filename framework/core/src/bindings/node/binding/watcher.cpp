...
inline bool GetBypassRestore(const Napi::CallbackInfo &info) {
  ...
  void WatcherAutoClient::connect(const event_ptr &event, const longfist::types::Register &register_data) {
    ...
    if (bypass_trading_data_) {
      ...
      if (app_location->category == category::MD and should_connect_md(app_location)) {
        ...
        SPDLOG_INFO("resume {} connection from {}", app_.get_location_uname(app_uid),
                    time::strftime(resume_time_point));
      }
  ...
  Napi::Value Watcher::IssueBasketOrder(const Napi::CallbackInfo &info) {
    ...
    if (GetBigInt(basket_order_info.Get("volume")) == VOLUME_ZERO) {
      basket_order_info.Set("calculation_mode",
                            Napi::Number::New(info.Env(), int(BasketOrderCalculationMode::Dynamic)));
    } else {
  ...
  void Watcher::on_start() {
    ...
    events_ | is(StrategyStateUpdate::tag) |
        $$(UpdateStrategyState(event->source(), event->data<StrategyStateUpdate>()));
