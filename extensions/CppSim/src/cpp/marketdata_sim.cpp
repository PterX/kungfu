...
MarketDataSim::MarketDataSim(broker::BrokerVendor &vendor)
    : MarketData(vendor) {
  KUNGFU_SETUP_LOG();
...
void MarketDataSim::on_start() {
  ...
  add_time_interval(int64_t(config_.tick * time_unit::NANOSECONDS_PER_SECOND),
                    [&](const auto &) { generate_tick(); });
}

bool MarketDataSim::subscribe(
    const std::vector<InstrumentKey> &instrument_keys) {
...
bool MarketDataSim::subscribe_custom(
    const longfist::types::CustomSubscribe &custom_sub) {
  return true;
}

std::string
MarketDataSim::make_exchange_instrument(const std::string &exchange_id,
...
void MarketDataSim::generate_quote() {
  for (auto &pair : map_quote_) {
    ...
    auto &bids_map_ = bid_orderbooks_[make_exchange_instrument(
        quote.exchange_id, quote.instrument_id)];
    auto &asks_map_ = ask_orderbooks_[make_exchange_instrument(
        quote.exchange_id, quote.instrument_id)];
    auto bid_iter = bids_map_.rbegin();
    auto ask_iter = asks_map_.begin();
    quote.last_price = map_last_price_[make_exchange_instrument(
        quote.exchange_id, quote.instrument_id)];
    quote.volume = map_volume_[make_exchange_instrument(quote.exchange_id,
                                                        quote.instrument_id)];
    for (int i = 0; i < 10; ++i) {
...
void MarketDataSim::write_transaction_trade(const Entrust &entrust,
                                            double volume, int64_t order_no) {
  auto iter = map_transaction_.try_emplace(
      make_exchange_instrument(entrust.exchange_id, entrust.instrument_id));
  ...
  transaction.instrument_type =
      get_instrument_type(entrust.exchange_id, entrust.instrument_id);
  transaction.volume = volume;
  if (entrust.side == Side::Buy) {
    transaction.price = map_ask_entrust_[make_exchange_instrument(
        entrust.exchange_id, entrust.instrument_id)][order_no]
                            .price;
    transaction.side = Side::Sell;
    transaction.ask_no = order_no;
    transaction.bid_no = 0;
  } else {
    transaction.price = map_bid_entrust_[make_exchange_instrument(
        entrust.exchange_id, entrust.instrument_id)][order_no]
                            .price;
...
void MarketDataSim::generate_tick() {
  ...
  if (transaction_band_writer_) {
    for (auto &pair : map_transaction_) {
      ...
      auto &bids_map_ = bid_orderbooks_[make_exchange_instrument(
          transaction.exchange_id, transaction.instrument_id)];
      auto &asks_map_ = ask_orderbooks_[make_exchange_instrument(
          transaction.exchange_id, transaction.instrument_id)];
      auto &bid_entrusts_ = map_bid_entrust_[make_exchange_instrument(
          transaction.exchange_id, transaction.instrument_id)];
      auto &ask_entrusts_ = map_ask_entrust_[make_exchange_instrument(
          transaction.exchange_id, transaction.instrument_id)];

      while (bids_map_.size() > 20 || asks_map_.size() > 20) {
        Side remove_side =
            bids_map_.size() > asks_map_.size() ? Side::Buy : Side::Sell;
        int64_t size = bids_map_.size() > asks_map_.size()
                           ? bid_entrusts_.size() - 1
                           : ask_entrusts_.size() - 1;
        std::uniform_int_distribution<int> dist_entrust(0, size);
        auto it = bids_map_.size() > asks_map_.size() ? bid_entrusts_.begin()
                                                      : ask_entrusts_.begin();
  ...
  if (entrust_band_writer_ and transaction_band_writer_) {
    for (auto &pair : map_entrust_) {
      ...
      entrust.orig_order_no = (entrust_band_writer_->current_frame_uid() ^
                               (entrust.data_time & 0xFFFFFFFF)) &
                              0x7FFFFFFFFFFFFFFF;
      SPDLOG_DEBUG("Entrust: {}", entrust.to_string());
      entrust_band_writer_->write(now(), entrust);
      if (entrust.side == Side::Buy) {
        map_bid_entrust_[make_exchange_instrument(entrust.exchange_id,
                                                  entrust.instrument_id)]
                        [entrust.orig_order_no] = entrust;
      } else {
        map_ask_entrust_[make_exchange_instrument(entrust.exchange_id,
                                                  entrust.instrument_id)]
                        [entrust.orig_order_no] = entrust;
      }
    }
    for (auto &pair : map_entrust_) {
      auto &entrust = pair.second;
      auto &bids_map_ = bid_orderbooks_[make_exchange_instrument(
          entrust.exchange_id, entrust.instrument_id)];
      auto &asks_map_ = ask_orderbooks_[make_exchange_instrument(
          entrust.exchange_id, entrust.instrument_id)];
      auto &bid_entrusts_ = map_bid_entrust_[make_exchange_instrument(
          entrust.exchange_id, entrust.instrument_id)];
      ...
      if (entrust.side == Side::Buy) {
        if (bids_map_.find(entrust.price) != bids_map_.end()) {
          ...
          bids_map_[entrust.price].order_no_vector.emplace_back(
              entrust.orig_order_no);
          bids_map_[entrust.price].data_time = entrust.data_time;
        } else {
          while (!asks_map_.empty() &&
                 asks_map_.begin()->first <= entrust.price && volume > 0) {
            ...
            for (int64_t order_no : vec) {
              ...
              if (ask_entrusts_[order_no].volume <= volume) {
                map_volume_[make_exchange_instrument(entrust.exchange_id,
                                                     entrust.instrument_id)] =
                    ask_entrusts_[order_no].volume;
                map_last_price_[make_exchange_instrument(
                    entrust.exchange_id, entrust.instrument_id)] =
                    ask_entrusts_[order_no].price;
                write_transaction_trade(entrust, ask_entrusts_[order_no].volume,
                                        order_no);
                to_remove.emplace_back(order_no);
                asks_map_.begin()->second.volume -=
                    ask_entrusts_[order_no].volume;
                volume -= ask_entrusts_[order_no].volume;
                bid_entrusts_[entrust.orig_order_no].volume -=
                    ask_entrusts_[order_no].volume;
                ask_entrusts_.erase(order_no);
              } else {
                map_volume_[make_exchange_instrument(
                    entrust.exchange_id, entrust.instrument_id)] = volume;
                map_last_price_[make_exchange_instrument(
                    entrust.exchange_id, entrust.instrument_id)] =
                    ask_entrusts_[order_no].price;
            ...
            for (int64_t order_no : to_remove) {
              vec.erase(std::remove(vec.begin(), vec.end(), order_no),
                        vec.end());
            }
            if (vec.empty()) {
          ...
          if (volume != 0) {
            ...
            bids_map_[entrust.price] =
                Level(entrust.price, volume, entrust.data_time, vector);
          }
        }
      } else {
        if (asks_map_.find(entrust.price) != asks_map_.end()) {
          asks_map_.at(entrust.price).volume += volume;
          asks_map_[entrust.price].order_no_vector.emplace_back(
              entrust.orig_order_no);
          asks_map_.at(entrust.price).data_time = entrust.data_time;
        } else {
          while (!bids_map_.empty() &&
                 bids_map_.rbegin()->first >= entrust.price && volume > 0) {
            ...
            for (int64_t order_no : vec) {
              ...
              if (bid_entrusts_[order_no].volume <= volume) {
                ...
                write_transaction_trade(entrust, bid_entrusts_[order_no].volume,
                                        order_no);
                to_remove.emplace_back(order_no);
                bids_map_.rbegin()->second.volume -=
                    bid_entrusts_[order_no].volume;
                volume -= bid_entrusts_[order_no].volume;
                ask_entrusts_[entrust.orig_order_no].volume -=
                    bid_entrusts_[order_no].volume;
