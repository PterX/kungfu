...
BackTestStreamDataBatcher::get_locations(int64_t begin_time, const std::string &instrument_id,
  ...
  while (slice_begin_time <= app_.now()) {
    auto location = from_indexer_->find_md_slice_location(slice_begin_time, app_.get_home()->group,
                                                          app_.get_home()->name, instrument_id, exchange_id, data_tag);
    ...
    slice_begin_time =
        1 + from_indexer_->get_md_slice_end_time(slice_begin_time, app_.get_home()->group, app_.get_home()->name,
