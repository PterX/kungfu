...
const run = (distDir, distName = 'api') => {
  ...
  return new Promise((resolve, reject) => {
    ...
    pack(jsApiConfig(argv))
      ...
      .catch((err) => {
        spinner.error('api');
        console.log(`\n  ${errorLog}failed to build api process`);
        console.error(`\n${err}\n`);
        process.exit(1);
      });
  });
...
if (require.main === module) {
  const defaultDistDir = getApiDefaultDistDir();
