...
void MultiCrossSectionalFactor::on_quote(const Quote &quote) {
  ...
  if (ask1_price == 0 || bid1_price == 0) {
    SPDLOG_WARN("Invalid quote in instrument={} at={}, ask1_price={}, bid1_price={}", quote.instrument_id, time::strftime(quote.data_time), ask1_price, bid1_price);
    return;
  }
...
std::map<std::string, CrossSection> MultiCrossSectionalFactor::generate_cross_sectional_factor(bool clear_price_cache, bool clear_factor_cache) {
  ...
  for (const auto & [instrument_key, time_stamp_price] : price_cache_) {
    if (_now - time_stamp_price.time > 10 * time_unit::NANOSECONDS_PER_SECOND) {
      SPDLOG_WARN("Price is too old, instrument_key={}, last update time={}, now={}", instrument_key, time::strftime(time_stamp_price.time), time::strftime(_now));
    }
