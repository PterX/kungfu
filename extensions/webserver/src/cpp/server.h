...
public:
  ...
  /*
  // just for test
  template <typename T1, typename T2> void test_fun_orderinput(const void *ptr, kungfu::yijinjing::webserver::stream_ptr
  stream) { auto data = static_cast<T1 *>(const_cast<void *>(ptr)); T2 data_send; memcpy(&data_send.data, data,
  sizeof(T1)); data_send.data.parent_id = kungfu::yijinjing::time::now_in_nano();
    //web_agent_->publish((char *)(&data_send), sizeof(T2), stream_id);
    stream->stream_send((char *)(&data_send), sizeof(T2));
    return;
  };

  template <typename T1, typename T2> void test_fun_order(const void *ptr, kungfu::yijinjing::webserver::stream_ptr
  stream) { auto data = static_cast<T1 *>(const_cast<void *>(ptr)); T2 data_send; memcpy(&data_send.data, data,
  sizeof(T1)); data_send.data.parent_id = kungfu::yijinjing::time::now_in_nano();
    //web_agent_->publish((char *)(&data_send), sizeof(T2), stream_id);
    stream->stream_send((char *)(&data_send), sizeof(T2));
  ...
  template <typename T1, typename T2> void test_fun_trade(const void *ptr, kungfu::yijinjing::webserver::stream_ptr
  stream) { auto data = static_cast<T1 *>(const_cast<void *>(ptr)); T2 data_send; memcpy(&data_send.data, data,
  sizeof(T1)); data_send.data.parent_order_id = kungfu::yijinjing::time::now_in_nano();
    //web_agent_->publish((char *)(&data_send), sizeof(T2), stream_id);
    stream->stream_send((char *)(&data_send), sizeof(T2));
    return;
  };
