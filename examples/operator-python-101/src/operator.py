...
def post_start(context):
    ...
    for v in baskets.values():
        if v.type == BasketType.ETF and v.min_volume > 0:
            ...
            basket_df = pd.concat(
                [basket_df, pd.DataFrame([basket])], ignore_index=True
            )
    ...
    for v in baskets_instruments.values():
        if (
            v.basket_uid in basket_df["id"].values
            and v.exchange_id != "Unkown"
            and v.volume > 0
        ):
            i += 1
            if i < 10000:
    ...
    context.add_time_interval(
        3 * 10 ** (9), lambda ctx, event: calculte_iopv_and_publish(ctx)
    )
...
def calculte_iopv_and_publish(context):
    ...
    inst_df["value"] = np.where(
        (inst_df["replace_flag"] == 2)
        | (inst_df["replace_flag"] == 4)
        | (inst_df["replace_flag"] == 6),
        inst_df["replace_balance"],
        inst_df["last_price"] * inst_df["volume"],
    )
    inst_df["buy_commission"] = np.where(
        ...
        0,
        inst_df.apply(
            lambda row: max(row["last_price"] * row["volume"] * sb_commission, 5)
            + row["last_price"] * row["volume"] * 0.00001,
            axis=1,
        ),
    )
    inst_df["sell_commission"] = np.where(
        ...
        inst_df.apply(
            lambda row: max(row["last_price"] * row["volume"] * sb_commission, 5)
            + row["last_price"] * row["volume"] * 0.00101,
            axis=1,
        ),
    )
    ...
    basket_df["inst_value"] = (
        basket_df["id"].map(inst_value).fillna(basket_df["inst_value"])
    )
    basket_df["inst_sell_commssion"] = (
        basket_df["id"]
        .map(inst_sell_commssion)
        .fillna(basket_df["inst_sell_commssion"])
    )
    basket_df["inst_buy_commssion"] = (
        basket_df["id"].map(inst_buy_commssion).fillna(basket_df["inst_buy_commssion"])
    )
    basket_df["inst_update_time"] = (
        basket_df["id"].map(inst_update_time).fillna(basket_df["inst_update_time"])
    )

    basket_df["pr_commmission"] = (
        basket_df["last_price"] * basket_df["min_volume"] * pr_commission
    )
    basket_df["buy_commission"] = basket_df.apply(
        lambda row: max(row["last_price"] * row["min_volume"] * sb_commission, 5)
        + row["last_price"] * row["min_volume"] * 0.00001,
        axis=1,
    )
    basket_df["sell_commission"] = basket_df.apply(
        lambda row: max(row["last_price"] * row["min_volume"] * sb_commission, 5)
        + row["last_price"] * row["min_volume"] * 0.00101,
        axis=1,
    )

    basket_df["iopv"] = (
        basket_df["inst_value"] + basket_df["cash_difference"]
    ) / basket_df["min_volume"]
    basket_df["discount"] = (
        basket_df["inst_value"]
        + basket_df["cash_difference"]
        - basket_df["last_price"] * basket_df["min_volume"]
        - basket_df["pr_commmission"]
        - basket_df["buy_commission"]
        - basket_df["inst_sell_commssion"]
    )
    basket_df["premium"] = (
        basket_df["last_price"] * basket_df["min_volume"]
    ...
    for index, row in basket_df.iterrows():
        ...
        if row["last_price"] > 0:
            sy_data = row.loc[
                ["last_price", "iopv", "discount", "premium", "inst_update_time"]
            ]
            context.publish_synthetic_data(
                str(int(row["id"])), json.dumps(sy_data.to_dict())
            )
