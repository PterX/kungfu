...
public:
  ...
  journal::writer_ptr open_hook_writer(uint32_t dest_id, const journal::hook_ptr &hook);

  journal::writer_ptr open_hook_writer_at(const data::location_ptr &location, uint32_t dest_id,
                                          const journal::hook_ptr &hook);
